...
public abstract class AbstractSipApplicationSessionBeanStore extends AttributeBeanStore
{

	   private static final LocLogger log = loggerFactory().getLogger(CONTEXT);

	   protected abstract SipApplicationSession getSession(boolean create);

	   public AbstractSipApplicationSessionBeanStore(NamingScheme namingScheme)
	   {
	      super(namingScheme);
	   }
	   ...
	   {
	      SipApplicationSession session = getSession(false);
	      if (session == null)
	      {
	         return emptyList();
	      }
	   ...
	   {
	      SipApplicationSession session = getSession(true);
	      if (session != null)
	      {
