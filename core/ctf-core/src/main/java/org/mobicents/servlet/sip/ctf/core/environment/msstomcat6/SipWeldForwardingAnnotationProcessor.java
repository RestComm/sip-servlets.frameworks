...
public class SipWeldForwardingAnnotationProcessor extends SipForwardingAnnotationProcessor
{
	...
	public SipWeldForwardingAnnotationProcessor(AnnotationProcessor originalAnnotationProcessor, AnnotationProcessor weldProcessor)
	{
	...
	private static SipWeldForwardingAnnotationProcessor createInstance(WeldManager manager, StandardContext stdContext)
	{
		try
		{
			Class<?> clazz = Reflections.classForName(SipWeldAnnotationProcessor.class.getName());
			AnnotationProcessor weldProcessor = (AnnotationProcessor) clazz.getConstructor(WeldManager.class).newInstance(manager);
			return new SipWeldForwardingAnnotationProcessor(getAnnotationProcessor(stdContext), weldProcessor);
		}
		catch (Exception e)
		{
	...
	public static void restoreAnnotationProcessor(ServletContextEvent sce)
	{
		...
		if (ap instanceof SipWeldForwardingAnnotationProcessor)
		{
			setAnnotationProcessor(stdContext,((SipWeldForwardingAnnotationProcessor)ap).firstProcessor);
		}
	}
	...
	/* (non-Javadoc)
	 * @see org.mobicents.servlet.sip.annotations.SipAnnotationProcessor#setContext(javax.naming.Context)
	 */
	@Override
	public void setContext(Context envContext) {	
		if (firstProcessor instanceof SipAnnotationProcessor){
			((SipAnnotationProcessorImpl)firstProcessor).setContext(envContext);
		}
	}

	/* (non-Javadoc)
	 * @see org.mobicents.servlet.sip.annotations.SipAnnotationProcessor#setSipContext(org.mobicents.servlet.sip.startup.SipContext)
	 */
	@Override
	public void setSipContext(SipContext context) {
		if (firstProcessor instanceof SipAnnotationProcessor){
			((SipAnnotationProcessorImpl)firstProcessor).setSipContext(context);
		}
	}
}
